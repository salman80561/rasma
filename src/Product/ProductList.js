...
const ProductList = (props) => {
    ...
    async function onAddHandler() {
        ...
        if (sameItem) {
            ...
            try {
                const response = await axios.put(`https://ecommerce-react-bfa19-default-rtdb.firebaseio.com/${ctx.email}/${sameItem.id}.json`, productObj)
                ctx.onShowCart()
                setIsAdded(true)
            ...
            try {
                const response = await axios.post(`https://ecommerce-react-bfa19-default-rtdb.firebaseio.com/${ctx.email}.json`, productObj)
                ctx.onShowCart()
                setIsAdded(true)
                console.log(response.data)
