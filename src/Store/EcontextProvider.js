...
const EcontextProvider = (props) => {
    ...
    const OnRemoveHandler = async (id) => {
        ...
        try {
            const response = await axios.delete(`https://ecommerce-react-bfa19-default-rtdb.firebaseio.com/${email}/${id}.json`)
        } catch (error) {
            alert(error.message)
    ...
    const onShowCart = useCallback(async () => {
        try {
            const response = await axios.get(`https://ecommerce-react-bfa19-default-rtdb.firebaseio.com/${email}.json`)
            const loadArr = [];
